...
@Component({
      ...
      <div class="profile-body" *ngIf="currentUser">
        ...
        <aside class="sidebar">
          ...
          <div class="card avatar-card">
            ...
            <div class="badge-row">
              <span class="badge badge-role">{{ currentUser.role | titlecase }}</span>
          ...
          <div class="card info-card">
            ...
            <div class="info-row">
              ...
              <div>
                ...
                <div class="info-val">{{ currentUser.created_at | date:'MMM d, yyyy' }}</div>
              </div>
            </div>
          </div>
        ...
        <main class="main-content">
          ...
          <div class="card">
            ...
            <div class="field">
              ...
              <div class="day-chips">
                <label *ngFor="let day of weekDays" class="day-chip"
                       [class.active]="editForm.workDays.includes(day)"
  ...
  styles: [`
    ...
    /* ── Sidebar ─────────────────────────────────────────────── */
    .sidebar { display: flex; flex-direction: column; gap: 12px; margin: 10px; }
...
.header-actions { 
  ...
  align-items: center;  /* Changed from 'right' to 'center' */
    ...
    .info-val { font-size: 12px; font-weight: 500; color: var(--text); margin-top: 1px; word-break: break-all; }
    ...
    .dayoff-hint { margin-top: 8px; font-size: 11px; color: var(--text-muted); }
...
export class ClientProfileComponent implements OnInit {
  ...
  getTableName(): string {
    return this.currentUser?.user_table === 'new_user' ? 'new_user' : 'users';
  }
  ...
  isOnLeave(): boolean {
    ...
    return this.leaveEntries.some(l => today >= new Date(l.startDate) && today <= new Date(l.endDate));
  }

  getLeaveEndDate(): string {
    const today = new Date(); today.setHours(0,0,0,0);
    return this.leaveEntries.find(l => today >= new Date(l.startDate) && today <= new Date(l.endDate))?.endDate || '';
  }

  getLeaveDuration(leave: { startDate: string; endDate: string }): number {
    return Math.ceil((new Date(leave.endDate).getTime() - new Date(leave.startDate).getTime()) / 86400000) + 1;
  }
